...
{
    /// <summary>
    /// Add this component to an object and it will show a XPbar above it
    /// You can either use a prefab for it, or have the component draw one at the start
    /// </summary>
    [AddComponentMenu("More Mountains/Tools/GUI/MMXPBar")]
    public class MMXPBar : MonoBehaviour 
	{
		/// the possible XP bar types
		public enum XPBarTypes { Prefab, Drawn }
        /// the possible timescales the bar can work on
        public enum TimeScales { UnscaledTime, Time }

		[MMInformation("Add this component to an object and it'll add a XPbar next to it to reflect its XP level in real time. You can decide here whether the XP bar should be drawn automatically or use a prefab.",MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
		/// whether the XPbar uses a prefab or is drawn automatically
		public XPBarTypes XPBarType = XPBarTypes.Drawn;
		...
		/// the prefab to use as the XP bar
		public MMProgressBar XPBarPrefab;

		[Header("Drawn XPbar Settings ")]
		[MMInformation("Set the size (in world units), padding, back and front colors of the XPbar.",MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
		/// if the XPbar is drawn, its size in world units
		public Vector2 Size = new Vector2(1f,0.2f);
		/// if the XPbar is drawn, the padding to apply to the foreground, in world units
		public Vector2 BackgroundPadding = new Vector2(0.01f,0.01f);
		/// the rotation to apply to the MMXPBarContainer when drawing it
		public Vector3 InitialRotationAngles;
        /// if the XPbar is drawn, the color of its foreground
        ...
        /// if the XPbar is drawn, the color of its delayed bar
        ...
        /// if the XPbar is drawn, the color of its border
        ...
        /// if the XPbar is drawn, the color of its background
        ...
        /// the name of the sorting layer to put this XP bar on
		...
		/// if this is true, bumps the scale of the XPbar when its value changes
        ...
        public bool NestDrawnXPBar = false;

		// [Header("Death")]
		// /// a gameobject (usually a particle system) to instantiate when the XPbar reaches zero
		// public GameObject InstantiatedOnDeath;

		[Header("Offset")]
		[MMInformation("Set the offset (in world units), relative to the object's center, to which the XP bar will be displayed.",MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
		/// the offset to apply to the XPbar compared to the object's center
		public Vector3 XPBarOffset = new Vector3(0f,5f,0f);

		[Header("Display")]
		[MMInformation("Here you can define whether or not the XPbar should always be visible. If not, you can set here how long after a hit it'll remain visible.",MoreMountains.Tools.MMInformationAttribute.InformationType.Info,false)]
		...
		/// <summary>
		/// On Start, creates or sets the XP bar up
		/// </summary>
		...
		public virtual void Initialization()
        {
            ...
            if (XPBarType == XPBarTypes.Prefab)
            {
                if (XPBarPrefab == null)
                {
                    Debug.LogWarning(this.name + " : the XPBar has no prefab associated to it, nothing will be displayed.");
                    return;
                }
                _progressBar = Instantiate(XPBarPrefab, transform.position + XPBarOffset, transform.rotation) as MMProgressBar;
                ...
                _progressBar.gameObject.name = "XPBar";
            }

            if (XPBarType == XPBarTypes.Drawn)
            {
                DrawXPBar();
                UpdateDrawnColors();
            }
		...
		/// Draws the XP bar.
		/// </summary>
		protected virtual void DrawXPBar()
		{
			GameObject newGameObject = new GameObject();
			SceneManager.MoveGameObjectToScene(newGameObject, this.gameObject.scene);
			newGameObject.name = "XPBar|"+this.gameObject.name;
			...
			_followTransform.Offset = XPBarOffset;
			...
			borderImageGameObject.name = "XPBar Border";
			...
			bgImageGameObject.name = "XPBar Background";
			...
			delayedImageGameObject.name = "XPBar Delayed Foreground";
			...
			frontImageGameObject.name = "XPBar Foreground";
		...
		/// <summary>
		/// On Update, we hide or show our XPbar based on our current status
		/// </summary>
		...
		// 	{
		// 		GameObject instantiatedOnDeath = Instantiate(InstantiatedOnDeath, this.transform.position + XPBarOffset, this.transform.rotation);
		...
		public virtual void UpdateBar(float currentXP, float minXP, float maxXP, bool show)
        {
            // if the XPbar isn't supposed to be always displayed, we turn it on for the specified duration
            ...
            if (_progressBar != null)
			{
				_progressBar.UpdateBar(currentXP, minXP, maxXP)	;
